...
- *Query 1*: #product_id's with price above the average selling price(61.48).These product_id's are: 121,127,125,122.

            select product_id,round((avg(sell_price)),2) as price from sales_1 group by product_id having price > 61.48 ;
            
- *Query 2*: #product_id's selling below the average selling price(61.48).These product_id's are 124,123.
